...
bool COpenAssetImportMesh::Load(const std::string& Filename)
{
    ...
    if (pScene) {
        ...
        errorMessage("Error loading mesh model", Importer.GetErrorString());
    }
...
bool COpenAssetImportMesh::InitMaterials(const aiScene* pScene, const std::string& Filename)
{
    ...
    std::string separator = "/";
#endif
    std::string::size_type SlashIndex = Filename.find_last_of(separator);
    std::string Dir;
    ...
    for (unsigned int i = 0 ; i < pScene->mNumMaterials ; i++) {
        ...
        if (pMaterial->GetTextureCount(aiTextureType_DIFFUSE) > 0) {
			...
			if (pMaterial->GetTexture(aiTextureType_DIFFUSE, 0, &Path, NULL, NULL, NULL, NULL, NULL) == AI_SUCCESS) {
                std::string FullPath = Dir + separator + Path.data;
                m_Textures[i] = new CTexture();
                if (!m_Textures[i]->Load(FullPath, true)) {
                    errorMessage("Error loading mesh texture", FullPath.c_str());
                    delete m_Textures[i];
                    m_Textures[i] = NULL;
                    Ret = false;
                }
